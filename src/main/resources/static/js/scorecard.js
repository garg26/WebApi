...
$('.ns-question-list')
		.each(
				function(index) {
					...
					$questionList
							.wrap('<form class="ns-question-list-form ns-question-list-form'
									+ index + '"></form>');
					...
					userDetailsHtml += '<div class="ns-row"><label></label><input type="hidden" name="paper_set" value="'
							+ (index + 1) + '"></input></div>';
					userDetailsHtml += '<div class="ns-row"><label>Email * : </label> <input type="email" name="user_email" value="" required></input></div>';
					userDetailsHtml += '<div class="ns-row"><label></label><input type="hidden" name="submit_date" value="'
							+ today + '"></input></div>';
					...
					$questionList
							.parent()
							.append(
									'<div class="ns-submit"><input type="submit" value="Check Score"></input></div>');
					$questionList
							.parent()
							.append(
									'<div class="ns-result">'
						...
						$questionList
								...
								.append(
										'<div class="insights-result">'
												...
												+ 'Total attempted: <span class="insights-attempted-questions"></span><br>'
					...
					/*
					 * $questionList.parent().append('<div
					 * class="iasbaba-result"><h4>IAS Baba <br>Answer Key
					 * Coming Soon!</h4></div>');
					 * $questionList.parent().append('<div
					 * class="insights-result"><h4>InsightsonIndia <br>Answer
					 * Key Coming Soon!</h4></div>');
					 */

					$(this).append(
							'<input type="hidden" name="list_id" value="'
									+ $('.ns-question-list').attr('id') + '">');

					$('.ns-question', $questionList).each(
							function() {
								var $question = jQuery(this);
								var questionId = $question.attr('data-id');

								$('.ns-answer', $question).each(
										function() {
					...
					$('.ns-question-list-form' + index + '')
							.on(
									...
									function(e) {
														...
														function() {
															...
															$('.ns-answer',
																	...
																	.removeClass(
																			'ns-answered-correct')
																	.removeClass(
																			'ns-answered-incorrect');
															var marksQuestion = parseFloat($question
																	.attr('data-marks'));
															var negativeMarksQuestion = parseFloat($question
																	.attr('data-neg-marks'));

															marksTotal += marksQuestion;
															totalQuestions++;

															$selectedAnswer = jQuery(
																	'input:checked',
																	$question);
															...
															if ($selectedAnswer.length > 0) {
																...
																if ($selectedAnswer
																	...
																	totalNegativeMarks_insights += negativeMarksQuestion;
																}
															}
														});
										 ...
										 * totalQuestions) { var submitConfirmed =
										 * confirm( "Are you sure you want to
										 * check your result?\n" + "# Total
										 * questions : " + totalQuestions + "\n" + "#
										 * Total questions attempted: " +
										 * attemptedQuestions + "\n" + "Press
										 * [OK/Yes] to proceed or [Cancel/No] to
										...
										totalMarksObtained = totalMarksObtained
												- totalNegativeMarks;
										totalMarksObtained_iasbaba = totalMarksObtained_iasbaba
												- totalNegativeMarks_iasbaba;
										totalMarksObtained_insights = totalMarksObtained_insights
												- totalNegativeMarks_insights;
										$('.ns-result .ns-score-obtained', this)
												.text(
														totalMarksObtained
																.toFixed(2));
										$('.ns-result .ns-score-total', this)
												.text(marksTotal);
										$('.ns-result .ns-correct-answers',
												this).text(correctAnswers);
										$('.ns-result .ns-incorrect-answers',
												this).text(incorrectAnswers);
										$('.ns-result .ns-total-questions',
												this).text(totalQuestions);
										$('.ns-result .ns-attempted-questions',
												this).text(attemptedQuestions);

										$(
												'.ns-result [name="ns_score_obtained"]',
												this).val(
												totalMarksObtained.toFixed(2));
										$('.ns-result [name="ns_score_total"]',
												this).val(marksTotal);
										$(
												'.ns-result [name="ns_correct_answers"]',
												this).val(correctAnswers);
										$(
												'.ns-result [name="ns_incorrect_answers"]',
												this).val(incorrectAnswers);
										$(
												'.ns-result [name="ns_total_questions"]',
												this).val(totalQuestions);
										$(
												'.ns-result [name="ns_attempted_questions"]',
												this).val(attemptedQuestions);

										$(
												'.iasbaba-result .iasbaba-score-obtained',
												this).text(
												totalMarksObtained_iasbaba
														.toFixed(2));
										$(
												'.iasbaba-result .iasbaba-score-total',
												this).text(marksTotal);
										$(
												'.iasbaba-result .iasbaba-correct-answers',
												this).text(
												correctAnswers_iasbaba);
										$(
												'.iasbaba-result .iasbaba-incorrect-answers',
												this).text(
												incorrectAnswers_iasbaba);
										$(
												'.iasbaba-result .iasbaba-total-questions',
												this).text(totalQuestions);
										$(
												'.iasbaba-result .iasbaba-attempted-questions',
												this).text(attemptedQuestions);

										$(
												'.iasbaba-result [name="iasbaba_score_obtained"]',
												this).val(
												totalMarksObtained_iasbaba
														.toFixed(2));
										$(
												'.iasbaba-result [name="iasbaba_score_total"]',
												this).val(marksTotal);
										$(
												'.iasbaba-result [name="iasbaba_correct_answers"]',
												this).val(
												correctAnswers_iasbaba);
										$(
												'.iasbaba-result [name="iasbaba_incorrect_answers"]',
												this).val(
												incorrectAnswers_iasbaba);
										$(
												'.iasbaba-result [name="iasbaba_total_questions"]',
												this).val(totalQuestions);
										$(
												'.iasbaba-result [name="iasbaba_attempted_questions"]',
												this).val(attemptedQuestions);

										$(
												'.insights-result .insights-score-obtained',
												this).text(
												totalMarksObtained_insights
														.toFixed(2));
										$(
												'.insights-result .insights-score-total',
												this).text(marksTotal);
										$(
												'.insights-result .insights-correct-answers',
												this).text(
												correctAnswers_insights);
										$(
												'.insights-result .insights-incorrect-answers',
												this).text(
												incorrectAnswers_insights);
										$(
												'.insights-result .insights-total-questions',
												this).text(totalQuestions);
										$(
												'.insights-result .insights-attempted-questions',
												this).text(attemptedQuestions);

										$(
												'.insights-result [name="insights_score_obtained"]',
												this).val(
												totalMarksObtained_insights
														.toFixed(2));
										$(
												'.insights-result [name="insights_score_total"]',
												this).val(marksTotal);
										$(
												'.insights-result [name="insights_correct_answers"]',
												this).val(
												correctAnswers_insights);
										$(
												'.insights-result [name="insights_incorrect_answers"]',
												this).val(
												incorrectAnswers_insights);
										$(
												'.insights-result [name="insights_total_questions"]',
												this).val(totalQuestions);
										$(
												'.insights-result [name="insights_attempted_questions"]',
												this).val(attemptedQuestions);
										...
										// $.get('https://script.google.com/a/neostencil.com/macros/s/AKfycbzs7W-upmTZv5SVXE0d1XC7JcsVSTtXzXGFCigauPKwHOH1r76V/exec',
										// formData, function(resp) {
										// });
										var user_name = $(".ns-user-details")
												.find("input[name='user_name']")
												.val();
										var user_email = $(".ns-user-details")
												.find(
														"input[name='user_email']")
												.val();
										var user_phone = $(".ns-user-details")
												.find(
														"input[name='user_phone']")
												.val();
										var user_subject = $(".ns-user-details")
												.find(
														"input[name='user_subject']")
												.val();
										/*
										 * $('.hbspt-form').css({"position":
										 * "absolute", "visibility":
										 * "hidden","top":"0"});
										 */
										$('.hbspt-form')
												...
												.find('form')
												.find(
														'#name-bcd0de48-792a-47fb-bb85-0bd3503542c7')
												.val(user_name);
										$('.hbspt-form')
												.find('iframe')
												.contents()
												.find('form')
												.find(
														'#email-bcd0de48-792a-47fb-bb85-0bd3503542c7')
												.val(user_email);
										$('.hbspt-form')
												.find('iframe')
												.contents()
												.find('form')
												.find(
														'#phone-bcd0de48-792a-47fb-bb85-0bd3503542c7')
												.val(user_phone);
										$('.hbspt-form')
												.find('iframe')
												.contents()
												.find('form')
												.find(
														'#optional_exam-bcd0de48-792a-47fb-bb85-0bd3503542c7')
												.val(user_subject);
										$('.hbspt-form')
												.find('iframe')
												.contents()
												...
												.find(
														'#name-bcd0de48-792a-47fb-bb85-0bd3503542c7')
												.val(user_name);
										totalMarksObtained_hubspot = totalMarksObtained;
										$('.hbspt-form')
												.find('iframe')
												.contents()
												.find('form')
										...
										$
												.ajax({
													url : "https://script.google.com/a/neostencil.com/macros/s/AKfycbzs7W-upmTZv5SVXE0d1XC7JcsVSTtXzXGFCigauPKwHOH1r76V/exec",
													data : JSON.stringify($(
															'.ns-question-list-form'
																	+ index
																	+ '')
															.serialize()),
													dataType : 'jsonp',
										...
										if (window.location.href
													...
													.each(
															function() {
																if ($(this)
																			...
																			.each(
																					function() {
																						...
																						if ($(
																								...
																								.parent()
																								.find(
																										'.ns-correct.iasbaba-correct.insights-correct').length > 0) {
																						} else {
																	...
																	if (nsCorrect != '') {
																		htmlToShowDifference = htmlToShowDifference += '<tr><td>'
																				+ questionNumber
																				+ '</td><td>'
																				+ nsCorrect
																				+ '</td><td>'
											...
											$('#differences')
													.html(
															'<h4>Attempted questions in which answer keys differ:</h4><table><thead><tr><th>Question no</th><th>NeoStencil</th><th>IAS Baba</th><th>Insights on India</th><th>Your Answer</th></tr></thead><tbody>'
																	+ htmlToShowDifference
																	+ '</tbody></table>');
										}
										/* $('#differences').html(''); */
